...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toSeq.groupBy(w => w.toLower).map(c => c._1 -> c._2.size).toList.sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    dictionary.map(w => wordOccurrences(w) -> w).groupBy(_._1).map(p => p._1 -> p._2.map(_._2))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    x.toMap.map(p => p._1 -> (p._2 - yMap(p._1))).filter(_._2 > 0).toList.sortBy(_._1)
  }
