...
class HuffmanSuite {
  ...
  @Test def `times loooool`: Unit =
    assertEquals(times(string2Chars("loooool")), List('l' -> 2, 'o' -> 5))
  ...
  @Test def `make ordered leaf list for some frequency`: Unit =
    assertEquals(List(Leaf('e', 1), Leaf('t', 2), Leaf('x', 3), Leaf('k', 7)), makeOrderedLeafList(List(('t', 2), ('e', 1), ('k', 7), ('x', 3))))
