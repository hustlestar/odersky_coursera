...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val tmpAcc = if (p(elem)) acc.incl(elem) else acc
    right.filterAcc(p, left.filterAcc(p, tmpAcc))
  }
  ...
  override def mostRetweeted: Tweet = {
    def compareTwoTweets(a: Tweet, b: Tweet): Tweet = {
      if (a.retweets > b.retweets) a else b
    }
    if (left.isEmpty && !right.isEmpty)
      compareTwoTweets(right.mostRetweeted, elem)
    else if (right.isEmpty && !left.isEmpty)
      compareTwoTweets(elem, left.mostRetweeted)
    else if (!right.isEmpty && !left.isEmpty)
      compareTwoTweets(left.mostRetweeted, right.mostRetweeted)
    else
      elem
  }
  ...
  override def descendingByRetweet: TweetList = {
    val retweeted = this.mostRetweeted
